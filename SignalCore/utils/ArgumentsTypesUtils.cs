...
public static class ArgumentsTypesUtils
{
    ...
    public static object? CastOrThrow(this object? v, Type t,Exception? e = null)
    {
        ...
        //If proper instance, return v
        if (t.IsInstanceOfType(v)) return v;

        // If we can assign from type, just return it
        ...
        //If json element, deserialize it
        if (v is JsonElement jsonElement)
            return jsonElement.Deserialize(t) ?? throw new ArgumentException($"Failed to convert {v} to type {t.Name}");

        //If we can convert from string representation, do this
        var parsed = ParseValue(t,v?.ToString() ?? "");
        if(parsed.GetType().Name==t.Name) return parsed;
